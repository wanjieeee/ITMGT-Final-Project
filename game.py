...
class Game:
    ...
    def start_game(self):
        ...
        # chess board offset
        self.board_offset_x = (self.screen.get_width() - 640) // 2 #center the board
        ...
        while self.running:
            self.clock.tick(30)
            delta_time = self.clock.get_time() / 1000.0  # time in seconds
            self.chess.update_timers(delta_time)
            # poll events
    ...
    def menu(self):
        ...
        start_btn = pygame.Rect(350, 300, 100, 50)
        # coordingates for "Exit" button
        exit_btn = pygame.Rect(350, 375, 100, 50)
        # show play button
        pygame.draw.rect(self.screen, black_color, start_btn)
        # show exit button
        pygame.draw.rect(self.screen, black_color, exit_btn)

        # white color
        white_color = (255, 255, 255)
        # create fonts for texts
        big_font = pygame.font.SysFont("argent", 50)
        small_font = pygame.font.SysFont("argent", 20)
        # create text to be shown on the game menu
        welcome_text = big_font.render("Chess", False, black_color)
        created_by = small_font.render("Created by Crisostomo, Lim, Tan", True, black_color)
        start_btn_label = small_font.render("Play", True, white_color)
        exit_btn_label = small_font.render("Exit", True, white_color)
        ...
        self.screen.blit(exit_btn_label, 
                      ((exit_btn.x + (exit_btn.width - exit_btn_label.get_width()) // 2, 
                      exit_btn.y + (exit_btn.height - exit_btn_label.get_height()) // 2)))
        ...
        if util.left_click_event():
            ...
            # check if enter or return key was pressed
                ...
                sself.menu_showed = False
                self.chess.reset()
                self.chess.winner = ""

            # Check if "Exit" button was clicked
            elif exit_btn.collidepoint(mouse_coords[0], mouse_coords[1]):
                pygame.draw.rect(self.screen, white_color, exit_btn, 3)
                pygame.quit()
                sys.exit()
    ...
    def declare_winner(self, winner):
        ...
        reset_btn = pygame.Rect(350, 300, 100, 50)
        # show reset button
